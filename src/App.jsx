...
function App() {
      ...
      try {
        ...
        if (dataNube && Array.isArray(dataNube)) {
          ...
          if (!isReady) {
            ...
            if (ult && ult.escenarios.length > 0) setEscenarios(ult.escenarios);
          }
        }
      } catch(e) {}
  ...
  useEffect(() => { recargarDatosDesdeNube(); }, []);
  ...
  // ─── LÓGICA DE CÁLCULO ACTUALIZADA SEGÚN TU PEDIDO ──────────────────────────
  const calcularPropuesta = () => {
    ...
    escenarios.forEach(e => {
      ...
      if (isStaff) { 
        // Lógica corregida: (Sueldo * % Laboral) + (Venta * % Indirecto)
        const sueldoCargas = (e.cantidad * e.sueldoBruto) * (1 + pctCostoLaboral/100);
        const indirectosVenta = (e.cantidad * e.ventaUnit) * (pctIndirectos/100);
        cFila = sueldoCargas + indirectosVenta;
      }
      else if (isWks) { 
        cFila = e.cantidad * e.costoDirecto; 
      }
      else { 
        // Para otros, asumimos que indirectos también pesan sobre la venta
        const base = e.cantidad * (p.costoFijo || 0); 
        const indirectosVenta = (e.cantidad * e.ventaUnit) * (pctIndirectos/100);
        cFila = base + indirectosVenta; 
      }
  ...
  const guardarEscenario = () => {
    ...
    setHistorial(p => [nR, ...p]); syncNube(nR, 'HistorialCompartido', '✅ Sincronizado OK');
  };
  ...
  return (
    <div className="p-4 sm:p-8 bg-gradient-to-br from-slate-50 to-purple-50 min-h-screen font-sans text-slate-900 overflow-x-hidden">
      <style>{`@media print { @page { size: landscape; margin: 10mm; } body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } }`}</style>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 items-center w-full lg:w-auto">
            {tienePermiso('busqueda') && <button onClick={() => setMostrarModalValores(true)} className="bg-white border border-purple-200 rounded-lg px-3 py-2 text-purple-600 hover:bg-purple-50 transition shadow-sm text-lg print:hidden shrink-0">🔍</button>}
            
            <button onClick={recargarDatosDesdeNube} disabled={isRefreshing} className={`bg-white border border-blue-200 rounded-lg px-3 py-2 text-blue-600 hover:bg-blue-50 transition shadow-sm text-sm font-bold uppercase flex items-center gap-1 print:hidden shrink-0 ${isRefreshing ? 'opacity-50' : ''}`}>
            ...
            <div className="bg-purple-100 px-3 py-2 rounded-lg text-[10px] sm:text-xs font-bold text-purple-700 flex flex-col items-center shrink-0 min-w-[110px] border border-purple-200 shadow-sm">
              ...
              <button onClick={() => setUsuarioActual(null)} className="text-red-500 hover:text-red-700 text-[8px] mt-0.5 uppercase tracking-wider font-black print:hidden">Cerrar Sesión</button>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              ...
              <tbody className="text-xs sm:text-sm">
                {escenarios.map(e => {
                  ...
                  if (p) { 
                    if(isStaff) { 
                      const sCargas = (e.cantidad * e.sueldoBruto) * (1 + pctCostoLaboral/100);
                      const iVenta = (e.cantidad * e.ventaUnit) * (pctIndirectos/100);
                      cTot = sCargas + iVenta;
                    } else if(isWks) { 
                      cTot = e.cantidad * e.costoDirecto; 
                    } else { 
                      const b = e.cantidad * (p.costoFijo || 0); 
                      const iVenta = (e.cantidad * e.ventaUnit) * (pctIndirectos/100);
                      cTot = b + iVenta;
                    } 
                  }
        ...
        {/* ... Resto del componente EERR y Objetivos se mantiene igual ... */}
        {mostrarHistorial && (
          <div className="bg-white rounded-xl shadow-sm border border-purple-100 overflow-hidden mb-6">
            <div className="p-3 sm:p-4 border-b border-purple-50 bg-gradient-to-r from-purple-50 to-pink-50 flex justify-between items-center"><h2 className="font-bold text-slate-700 text-xs sm:text-sm uppercase tracking-tighter">📋 Escenarios Guardados</h2><button onClick={() => setMostrarHistorial(false)} className="text-slate-400 text-[10px] uppercase font-bold print:hidden">Cerrar</button></div>
            <div className="p-3 sm:p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {historial.map(i => (
                  <div key={i.id} className="border border-purple-100 rounded-lg p-3 hover:border-purple-400 transition bg-white shadow-sm flex flex-col justify-between">
                    <div>
                      <h3 className="font-black text-purple-700 text-xs uppercase truncate mb-1">{i.nombre}</h3>
                      <p className="text-[9px] text-slate-400 mb-2">{i.fecha}</p>
                    </div>
                    <button onClick={() => cargarEscenarioDesdeHistorial(i)} className="w-full bg-purple-50 text-purple-700 py-1.5 rounded font-black text-[9px] uppercase hover:bg-purple-600 hover:text-white transition">Cargar</button>
                  </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-purple-200 overflow-hidden mb-6">
          <div className="p-3 sm:p-4 border-b border-purple-100 flex justify-between items-center bg-gradient-to-r from-purple-600 to-pink-600 text-white"><h2 className="font-bold text-xs sm:text-sm">📊 Estado de Resultados Comparativo</h2><button onClick={() => setMostrarEERR(!mostrarEERR)} className="bg-white/20 px-2 py-1 rounded text-[9px] font-black uppercase transition print:hidden">{mostrarEERR ? '✕' : '👁️'}</button></div>
          {mostrarEERR && (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-[10px] sm:text-xs min-w-[700px]">
                <thead>
                  <tr className="bg-purple-50 text-purple-600 font-bold uppercase text-[9px]">
                    <th className="p-2 border-r border-purple-100"></th><th className="p-2 text-right border-r border-purple-100">Base</th><th className="p-2 text-right border-r border-purple-100">%</th>
                    <th className="p-2 text-right bg-green-50 border-r border-green-200">Propuesta</th><th className="p-2 text-right bg-green-50 border-r border-green-200">%</th>
                    <th className="p-2 text-right bg-blue-50 border-r border-blue-200">Total</th><th className="p-2 text-right bg-blue-50">%</th>
                  </tr>
                </thead>
                <tbody>
